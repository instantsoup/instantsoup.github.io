...
import { freeSpellsAtWizardLevel, isFreeSource } from '../rules/wizard/spellbook';
...
type LevelsPanelProps = {
  ...
  /** Wizard spells live in the spellbook; wizard levels show what was learned at that level. */
      ...
      {levels.length > 0 && (
        <div className="levels-list">
          {levels.map((lvl, levelIndex) => {
            ...
            // Wizard levels don't keep their own spell list: what a wizard learns goes in the
            // spellbook (Build → Spellbook). This tab just shows the free spells learned here.
            ...
            const spellsToLearn = isWizard
              ? freeSpellsAtWizardLevel(classLevelAtThisPoint, intModifier)
              : null;
            const levelSpells = isWizard
              ? spellbook
                  .filter((e) => e.addedAtCharLevel === lvl.level && isFreeSource(e.source))
                  .map((e) => e.spellName)
              : (lvl.spells ?? []);

            const spellSearchResults = isWizard
              ? []
              : getSpellSearchResults(lvl.level, spellListKey, null);
                    ...
                    {isWizard && spellsToLearn !== null ? (
                      <>
                        <div className="level-spells-header">
                          ...
                          >
                            {levelSpells.length} / {spellsToLearn} free spells
                          </span>
                          <span className="level-spells-hint">
                            {classLevelAtThisPoint === 1
                              ? 'Starting spellbook: 3 + Int modifier 1st-level spells, plus every cantrip.'
                              : 'Two free spells of any level you can cast.'}{' '}
                            Add them in the Spellbook section.
                          </span>
                        </div>
                        {levelSpells.length > 0 && (
                      ...
                      <>
                        <div className="level-spells-search">
                          <input
                            ...
                            onChange={(e) =>
                              setSpellSearchTerms((prev) => ({
                                ...prev,
                                [lvl.level]: e.target.value,
                              }))
                            }
