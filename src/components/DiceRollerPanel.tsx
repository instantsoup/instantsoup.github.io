...
import { rollMany, formatPool } from '../lib/dice'
  ...
  return (
    <div>
      <div className="btn-row mb-8">
        {COMMON.map(s => (
          <button key={s} type="button" className="btn" onClick={() => addDie(s)} aria-label={`Add d${s}`}>
            {s}
          </button>
        ))}
      </div>
      ...
      <div className="btn-row">
        <button type="button" className="btn btn--primary" onClick={roll}>Roll</button>
        <button type="button" className="btn btn--danger" onClick={clearAll}>Clear</button>
      </div>

      {last && (
        <div className="dice__result">
          <div><strong>Total:</strong> {last.total}</div>
          {last.rolls.length > 0 && <div><strong>Rolls:</strong> {last.rolls.join(', ')}</div>}
        </div>
      )}
    </div>
  )
